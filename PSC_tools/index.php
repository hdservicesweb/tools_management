                        ...
                        <table class="table table-striped table-bordered table-condensed table-sm">
                            <thead>
                                <tr class="text-center">
                                    ...
                                    <th><small><i class="fa fa-calendar"></i></small></th>
                                    <th><small><i class="fa fa-user"></i></small></th>
                                    <th><small><i class="fa fa-cubes"></i></small></th>
                                </tr>
                                <tr>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                $sqllasttools = "SELECT TP.psc_id,TP.name,TP.date,TM.stock from tools_process TP INNER JOIN tools_main_db TM on TP.psc_id = TM.psc_id where TP.process = '1' order by date desc limit 5
                                ";
                                $wodata = mysqli_query($link, $sqllasttools) or die("Something wrong with DB please verify.");

                                while ($row = mysqli_fetch_array($wodata)) {
                                    printf("<tr><td>&nbsp;%s</td><td>&nbsp;%s&nbsp;</td><td>&nbsp;%s&nbsp;</td><td>&nbsp;%s&nbsp;</td></tr>", "<a href='tool_details.php?id=" . $row['psc_id'] . "'><small>" . $row['psc_id'] . "</small></a>", "<small>" . $row['date'] . "</small>", "<a href='show_me.php?selector=3&textfield=" . $row['name'] . "'><small>" . $row['name'] . "</small></a>", "<small><small><b>" . $row['stock'] . "</b></small></small>");
                                }
    ...
    <div class="container-fluid">
        ...
        if (mysqli_num_rows($result2) > 0) {
            ...
            while ($rowcommon = mysqli_fetch_array($result2)) {
                echo "<div class='col-sm-2' style='padding:5px'> ";
                echo "<div class='card ' > ";
                echo '<div class="row no-gutters">';
                if (($rowcommon['img'] != NULL) || ($rowcommon['img'] != "")) {
                    echo " <div class='col-xs-12 col-sm-4'>
                                <a href='" . $rowcommon['img'] . "' data-lightbox='image-1' data-title='" . $rowcommon["component_pn"] . "'> <img src='" . $rowcommon['img'] . "' class='card-img' onerror=this.onerror=null;this.src='tools_imgs/no_image.png'> </a>
                            </div>";
                } else {
                    echo " <div class='col-xs-12 col-sm-4'>
